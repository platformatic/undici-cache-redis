...
test('should stream cache entries', async t => {
  ...
  const store2 = await createStore(t, { prefix: prefix1, cacheTagsHeader: 'cache-tags' })
  const manager = await createManager(t)

  await waitForEvents(store1, 'entry:write', 3, async () => {
  ...
  await waitForEvents(store2, 'entry:write', 3, async () => {
  ...
  const dependentEntries = await (manager as Cache).getDependentEntries(entry.id, [prefix1, prefix2])
