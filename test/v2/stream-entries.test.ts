...
test('should stream cache entries', async t => {
  ...
  const store1 = await createStore(t, { prefix: prefix1, cacheTagsHeader: 'x-cache-tags' })
  const store2 = await createStore(t, { prefix: prefix2, cacheTagsHeader: 'x-cache-tags' })
  ...
  await waitForEvents(store1, 'entry:write', 1, async () => {
  ...
  await waitForEvents(store2, 'entry:write', 1, async () => {
