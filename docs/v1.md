...
await store.deleteKeys([{ origin: 'https://api.example.com', method: 'GET', path: '/users/123' }])
...
- `getDependentEntries(id, keyPrefix): Promise<CacheValueWithAdditionalProperties[]>` - Get entries sharing cache tags
