...
class TrackingCache {
  ...
  get (key) {
    ...
    process._rawDebug('--------TRACKING GET-------', key, entry?.result)
    return entry?.result
  }
  ...
  set (key, metadata, result) {
    process._rawDebug('--------TRACKING SET-------', key)
    const entry = this.#findMatchingEntry(key)
    if (entry !== undefined) {
   ...
   */
  delete (key) {
    process._rawDebug('--------TRACKING DELETE-------', key)
    const trackingMetadataKey = serializeTackingMetadataKey(key)
    ...
    process._rawDebug('--------TRACKING DELETE-------', entries.size)
    if (entries.size === 0) {
