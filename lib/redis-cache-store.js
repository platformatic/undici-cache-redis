...
class RedisCacheStore {
  ...
  constructor (opts) {
    ...
    this.#dataClient.keys(`${this.keyPrefix}values:*`)
      ...
      .catch(err => {
        process.emitWarning(`RedisCacheStore failed grabbing keys: ${err}`)
      })

    this.#subClient = new Redis(redisClientOpts)

    this.#subClient.subscribe(ENTRY_COUNT_UPDATE_CHANNEL).catch(err => {
      process.emitWarning(`RedisCacheStore failed subscribing to entry count updates: ${err}`)
    })
   ...
   */
  async close () {
    await Promise.allSettled([this.#dataClient.disconnect(), this.#subClient.disconnect()])
  }
  ...
  async createReadStream (req) {
    ...
    const metadataValuesString = await this.#dataClient.hget(
      metadataKey,
      'values'
    )
    ...
    const valueString = await this.#dataClient.get(metadataValue.key)
    if (!valueString) {
      ...
      this.#dataClient.hmset(metadataKey, {
        ...
        process.emitWarning(`RedisCacheStore error when removing expired value from metadata: ${err}`)
      })
  ...
  createWriteStream (req, opts) {
    ...
    writable.on('end', () => {
      ...
      const sendToRedis = async () => {
         ...
         * }}
         */
        let metadata = await this.#dataClient.hgetall(metadataKey)
        ...
        if (metadata.deleteAt) {
          ...
          if (value) {
            ...
            this.#dataClient.publish(ENTRY_COUNT_UPDATE_CHANNEL, `${this.#entryCount + 1}`)
              .catch(err => {
                process.emitWarning(`failed publishing entry count update: ${err}`)
              })
      ...
      sendToRedis().catch(err => {
        process.emitWarning(`RedisCacheStore error when writing to redis: ${err}`)
      })
    })
