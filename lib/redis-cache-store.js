...
* @returns {string}
*/
function generateEntryId (options) {
  const { keyPrefix, origin, path, method, vary } = options

  const encodedOrigin = encodeURIComponent(origin)
  const encodedPath = encodeURIComponent(path)

  let key = `${keyPrefix}:${encodedOrigin}:${encodedPath}:${method}:`

  if (vary) {
    const varyHeadersNames = Object.keys(vary).sort().map(
      header => header.toLowerCase()
    )
    for (const headerName of varyHeadersNames) {
      key += headerName
      const headerValue = vary[headerName]
      if (Array.isArray(headerValue)) {
        key += headerValue.sort().join(',')
      } else {
        key += headerValue
      }
    }
