...
class RedisCacheStore extends EventEmitter {
  ...
  async deleteTags (cacheTags) {
    ...
    for (const tag of cacheTags) {
      promises.push(deleteByTag(this.#context, tag))
    }
...
class RedisCacheManager extends EventEmitter {
  ...
  /**
   * @param {(entries: CacheEntry[]) => Promise<unknown> | unknown} callback
   */
  async streamEntries (callback) {
    await scanByPattern(this.#context, '*ids:*', async (keys) => {
      const promises = new Array(keys.length)
      ...
      await callback(entries.filter(entry => entry !== undefined))
    })
  }
   ...
   */
  async getValueById (keyPrefix, id) {
    return this.#redis.get(`${keyPrefix}values:${id}`)
  }
   ...
   * @returns {Promise<void>}
   */
  async deleteIds (keyPrefix, ids) {
    const promises = []
    for (const id of ids) {
      promises.push(this.#deleteById(keyPrefix, id))
    }
    await Promise.all(promises)
  }
