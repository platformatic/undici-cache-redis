...
class RedisCacheStore {
  ...
  async deleteRoutes (routes) {
    ...
    for (const { method, url } of routes) {
      ...
      promises.push(this.#execByPattern(`${key}*`, (pipeline, keys) => {
        for (const metadataKey of keys) {
          const { id } = this.#parseBaseMetadataKey(metadataKey)
          const valueKey = `${this.#makeBaseValueKey({ origin })}:${id}`

          pipeline.del(metadataKey.replace(this.#keyPrefix, ''))
  ...
  #execByPattern (pattern, callback) {
    ...
    return new Promise((resolve, reject) => {
      stream.on('data', keys => callback(pipeline, keys))
      stream.on('end', () => pipeline.exec(resolve))
