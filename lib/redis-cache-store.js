...
class RedisCacheStore extends EventEmitter {
  ...
  #subscribe () {
    ...
    this.#redisSubscribe.call('CLIENT', 'ID')
      .then(clientId => (
        this.#redis.call('CLIENT', 'TRACKING', 'on', 'REDIRECT', clientId)
      ))
...
class RedisCacheManager extends EventEmitter {
   ...
   * @type {boolean}
   */
  #configureKeyspaceEventNotification
  ...
  constructor (opts) {
    ...
    if (opts) {
      ...
      if (opts.configureKeyspaceEventNotification) {
        if (typeof opts.configureKeyspaceEventNotification !== 'boolean') {
          throw new TypeError('expected opts.configureKeyspaceEventNotification to be a boolean')
        }
        this.#configureKeyspaceEventNotification = opts.configureKeyspaceEventNotification
      }
    ...
    if (!this.#configureKeyspaceEventNotification) this.#configureKeyspaceEventNotification = false
  ...
  async subscribe () {
    ...
    try {
      // Managed Redis solutions, like Elasticache or MemoryStore, disable `CONFIG SET`
      // User's must enable the `notify-keyspace-events` setting through other means
      if (this.#configureKeyspaceEventNotification) {
