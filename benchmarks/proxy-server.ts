  ...
  handler: async (request, reply) => {
    try {
      ...
      if (cacheHit) {
        ...
        debug(`CACHE HIT: ${method} ${url} (${duration}ms, age: ${response.headers.age}s)`)
      } else {
        reply.header('x-proxy-cache', 'MISS')
        debug(`CACHE MISS: ${method} ${url} (${duration}ms)`)
      }
      ...
      reply.code(500)
      return { error: 'Proxy error', message: (error as Error).message }
    }
  }
})
...
const start = async () => {
  try {
    debug(`[${CACHE_TYPE.toUpperCase()}] Creating agent...`)
    agent = createAgent()
    debug(`[${CACHE_TYPE.toUpperCase()}] Starting proxy server on port ${PORT}...`)
    await app.listen({ port: Number(PORT), host: '0.0.0.0' })
    debug(`[${CACHE_TYPE.toUpperCase()}] Proxy server listening on http://0.0.0.0:${PORT}`)
  } catch (err) {
    debug(`[${CACHE_TYPE.toUpperCase()}] Server startup error:`, err)
    process.exit(1)
  }
}
